...
syspath = '/content/drive/MyDrive/bertcnn/keras-bert-/hotl'
os.chdir(syspath)
# 注意！！！！特别注意！！！此处要慎重，以下内容是Bert-Pre-training
config_path = '/content/drive/MyDrive/bertcnn/keras-bert-/hotl/Cbert_config.json'# 加载配置文件
checkpoint_path = '/content/drive/MyDrive/bertcnn/keras-bert-/hotl/Cbert_model.ckpt'
dict_path = '/content/drive/MyDrive/bertcnn/keras-bert-/hotl/Cvocab.txt'
#  以上是模型的加载，要慎重，一定要慎重！！！！！
...
def get_data():
    ...
    with codecs.open('./data/very_negative.txt','r','utf-8') as reader:
        for line in reader:
            very_neg.append(line.strip())
    with codecs.open('./data/negative.txt','r','utf-8') as reader:
        for line in reader:
            neg.append(line.strip())
    with codecs.open('./data/neural.txt','r','utf-8') as reader:
        for line in reader:
            neural.append(line.strip())
    with codecs.open('./data/positive.txt','r','utf-8') as reader:
        for line in reader:
            pos.append(line.strip())
    with codecs.open('./data/very_positive.txt','r','utf-8') as reader:
        for line in reader:
            very_pos.append(line.strip())
    return very_neg,neg,neural,pos,very_pos


# 得到编码
#def get_encode(pos,neg,token_dict):
def get_encode(very_neg,neg,neural,pos,very_pos,token_dict):
    '''
    :param pos:第一类文本数据
    :param neg:第二类文本数据
    ...
    '''
    all_data = very_neg+ neg + neural + pos + very_pos 
...
def train(wordvec,y):
    model = build_model()
    model.fit(wordvec,y,batch_size=16,epochs=2,validation_split=0.2) 
    #y是output
    model.summary()
...
if __name__ =='__main__':
    pos,neg,neural,very_pos,very_neg = get_data()
    token_dict = get_token_dict(dict_path)
    # get_encode()
    [X1,X2] = get_encode(very_neg,neg,neural,pos,very_pos,token_dict)
    wordvec = build_bert_model(X1,X2)
    # 标签类，其中选取3000个积极的文本和3000个消极的文本，将积极的记为1，将消极的记为0
    
    y = np.concatenate((np.ones(100, dtype=int), 2*np.ones(100, dtype=int),3*np.ones(100, dtype=int),4*np.ones(100, dtype=int),5*np.ones(100, dtype=int)))    
